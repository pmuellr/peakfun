...
function * mainGen (cb) {
  ...
  // run sample
  exec('node -r ./peakfun test/fixtures/run-a', {async: true, silent: true}, (code, stdout, stderr) => {
    if (code === 0) {
      return logger.log('ran sample')
    }

    logger.log('error running sample')
    logger.log(stdout)
    logger.log(stderr)
    logger.log(colorFail('----- running sample failed! -----'))
  })
}
